  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white" style={{ fontFamily: "var(--font-display)" }}>Tüm Ödemeler</h1>
          <p className="text-sm text-gray-400 mt-0.5">{payments.length} kayıt • Tahsilat: <span className="text-emerald-400 font-semibold">{formatPrice(totalSuccess)}</span></p>
        </div>
      </div>
        ...
        <div className="md:col-span-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Müşteri, mağaza, ödeme no…"
              value={searchInput}
            ...
            <tbody className="divide-y divide-gray-800">
              {loading ? (
                ...
                <tr key={p.id} className="hover:bg-gray-800/50">
                  ...
                  <td className="p-4 text-rose-400 font-medium">{p.tenant?.name}</td>
                  <td className="p-4 text-white">{p.appointment?.customer?.firstName} {p.appointment?.customer?.lastName}</td>
                  <td className="p-4 text-gray-300">{p.appointment?.service?.name}</td>
                  <td className="p-4 text-right text-white font-semibold">{formatPrice(p.amount)}</td>
